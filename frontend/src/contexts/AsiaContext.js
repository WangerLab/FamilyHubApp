...
const AsiaContext = createContext(null);

export const AsiaProvider = ({ children }) => {
  ...
  const [shoppingMode, setShoppingMode] = useState(
    () => localStorage.getItem('shoppingModeAsia') === 'true'
  );
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`asia:${member.household_id}`)
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'asia_items', filter: `household_id=eq.${member.household_id}` },
        (payload) => {
    ...
    if (checked && uncheckedBefore === 1 && items.length > 0) {
      if (activity?.logActivity) {
        activity.logActivity({
          ...
          module: 'asia',
          item_id: null,
          description: `${member.display_name} hat die Asia-Liste komplett erledigt 🎉`,
        });
      }
    }
  ...
  const clearList = async () => {
    ...
    const { error } = await supabase.rpc('archive_checked_asia');
    if (error) {
      console.warn('[asia] archive_checked_asia failed:', error.message);
      return;
    }
    await fetchItems();
  };
  ...
  const toggleShoppingMode = () => {
    setShoppingMode((prev) => {
      const next = !prev;
      localStorage.setItem('shoppingModeAsia', String(next));
      return next;
    });
  };
...
export const useAsia = () => useContext(AsiaContext);
